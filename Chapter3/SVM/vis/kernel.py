...
ax1.scatter(class1[:, 0], class1[:, 1], color='b', label='类别 1 (环)')
ax1.scatter(class2[:, 0], class2[:, 1], color='r', label='类别 2 (中心)')
ax1.set_xlabel('X1')
ax1.set_ylabel('X2')
ax1.set_title('环形的原始2D数据')
ax1.axis('equal')
ax1.legend()
...
# 增加透明度，使得重叠部分更清晰
ax2.scatter(X_pca[:100, 0], X_pca[:100, 1], z_values[:100], color='b', alpha=0.6, label='类别 1 (环)')
ax2.scatter(X_pca[100:, 0], X_pca[100:, 1], z_values[100:], color='r', alpha=0.6, label='类别 2 (中心)')
ax2.set_xlabel('X')
ax2.set_ylabel('Y')
ax2.set_zlabel('Z')
ax2.set_title('基于RBF升维的3D数据可视化')
