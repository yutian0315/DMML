...
# 绘制class1和class2的散点图
ax.scatter(class1[:, 0], class1[:, 1], class1[:, 2], color='b', label='类别 1')
ax.scatter(class2[:, 0], class2[:, 1], class2[:, 2], color='r', label='类别 2')
...
zz_margin1 = zz - margin / np.abs(svm.coef_[0][2])
zz_margin2 = zz + margin / np.abs(svm.coef_[0][2])

# 绘制超平面
ax.plot_surface(xx, yy, zz, color='g', alpha=0.5)

# 绘制支持向量边界
ax.plot_surface(xx, yy, zz_margin1, color='y', alpha=0.3, linestyle='dashed')
ax.plot_surface(xx, yy, zz_margin2, color='y', alpha=0.3, linestyle='dashed')

# 标签和标题
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_zlabel('Z')
ax.set_title('3D散点图中的最大间隔')
ax.legend()
